...
EXPOSURE_DATA_ID <- 'ieu-a-2'  # 数据标识符。建议使用英文简称，如 UK_Biobank_BMI 等
EXPOSURE_REFER_ID <- 'GRCh37' # GWAS 数据所用的参考序列版本，如 GRCh37 等
EXPOSURE_POP <- 'EUR'      # 人口学种群标识，如 AFR（非洲）、AMR（美洲）、EAS（东亚）、EUR（欧洲）、SAS（南亚）

OUTCOME_NAME <- 'CHD'
OUTCOME_DATA_ID <- 'ieu-a-7'
OUTCOME_REFER_ID <- 'GRCh37'
OUTCOME_POP <- 'EUR'
